...
- 强调样式为**暖色文字 + 半透明深色底片**(`--color-day-shift` / `--color-day-shift-bg`),不是单纯
  换个文字色:卡片背景是四种天色渐变,没有任何单一文字色在四种天色下都可读 —— 浅色在白天与黎明
  卡片上只有约 1.2:1(原橙色同样是 1.2),深色在夜晚卡片上只剩 1.0。加底片后最差约 3.4:1。
  底片用负 margin 抵消内边距,使日期右边缘与未强调的卡片对齐。
